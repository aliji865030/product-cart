...
const Products = [
    {id: 1, name: "Product-1", price:100,quantity:0,display:"none"},
    {id: 2, name: "Product-2", price:200,quantity:0,display:"none"},
    {id: 3, name: "Product-3", price:300,quantity:0,display:"none"},
    ];
...
function reducer(state, action) {
    ...
    if (action.type === "add") {
        ...
        const updatedProducts = state.product.map(item => {
            if (item.id === action.payLoad && item.quantity > 0) {
                total-=item.price;
                if(item.quantity==1){item.display="none"}
                return {
                    ...item,
                    quantity: item.quantity - 1
                };
    ...
    <div className='w-full h-screen flex justify-center items-center gap-20 '>
        ...
        <div className='w-5/12 bg-slate-600 flex flex-col justify-center items-center gap-10 py-10 text-2xl border-8 border-gray-300'>
            {state.product.map((item,index)=>{
                return(
                    <div key={index} className='flex justify-around gap-10 h-16 bg-slate-400 w-11/12 items-center'>
                        <span>{item.name}</span>
                        <span>{item.price}</span>
                        <span className='flex justify-center gap-3'>
                            <span className=' cursor-pointer' onClick={()=>{dispatch({type:"remove",payLoad:item.id})}}>-</span>
                            <span>{item.quantity}</span>
                            <span className=' cursor-pointer' onClick={()=>{dispatch({type:"add",payLoad:item.id})}}>+</span>
                        </span>
                    </div>
                )
            })}
        ...
        <div className='w-3/12 bg-slate-600  flex flex-col justify-center items-center gap-5 text-xl border-8 border-gray-300' >
                <div className='flex justify-center flex-col items-center gap-5 pt-7 w-full'  >
                    {state.product.map((item,index)=>{
                          return(
                            <div key={index} className='flex justify-around w-11/12 bg-slate-400 h-12 items-center'  >
                              <div>{item.name}</div>
